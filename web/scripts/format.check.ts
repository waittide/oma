...
// 不足 1 秒用毫秒：亚秒级信息不被抹成「0.0 s」
eq('0 ms', formatDuration(0), '0 ms');
eq('320 ms', formatDuration(320), '320 ms');
eq('999 ms', formatDuration(999), '999 ms');
// 1 秒到 10 秒保留一位小数，便于看清「刚发生」的那几秒
eq('1000 ms', formatDuration(1000), '1.0 s');
...
// 负数（时钟回拨、服务端时间戳异常）按 0 处理，不显示「-500 ms」
eq('negative', formatDuration(-500), '0 ms');
