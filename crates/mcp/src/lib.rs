...
use oma_contract::{McpServerConfig, ToolOutput};
use oma_tool::Tool;
...
mod local_stdio_e2e {
    ...
    /// 本地 stdio 服务端端到端：预热 → 工具注册（带命名空间）→ 真正调用一次。
    ///
    /// 用一个最小 Python MCP 服务端当对端：只实现 initialize / tools/list / tools/call，
    /// 足以覆盖握手、工具清单与调用回执三条路径。
    #[tokio::test]
    async fn test_local_stdio_discovery_and_call() {
...
for line in sys.stdin:
    ...
    if m == "initialize":
        ...
        out = {"tools": [{"name": "echo", "description": "echo", "inputSchema": {"type": "object"}}]}
    elif m == "tools/call":
        text = (req.get("params") or {}).get("arguments", {}).get("text", "")
        out = {"content": [{"type": "text", "text": "ECHO:" + text}], "isError": False}
    else:
        continue
...
"#,
        ...
        assert_eq!(names, vec!["mcp__demo__echo".to_string()], "工具名必须带命名空间");

        let out = tools[0]
            .execute(dir.path(), serde_json::json!({ "text": "hi" }))
            .await;
        assert!(!out.is_error, "{}", out.output);
        assert!(out.output.contains("ECHO:hi"), "{}", out.output);
    }
}
