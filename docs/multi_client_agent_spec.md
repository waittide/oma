...
- 具备功能：
  1. 会话列表管理与工作区选择；
  2. 树状对话流展示、Markdown 渲染、Thinking 思维链折叠。**助手侧按轮合并成一组**：
     一次工具循环会落多条助手消息，逐条重复模型名与统计行只是噪声，因此一组里
     顶部只有一个模型标签、中间是按到达顺序排开的思考/工具/正文、底部只有一行
     整轮合计（`耗时 <该轮墙钟> · 该轮各次请求的用量之和`）；**每个思考段与每次工具
     调用各自在折叠按钮左侧显示自己的耗时**（`thinking.duration_ms` / 回执里的
     `duration_ms`，都由服务端测量）。流式中的那一轮同样带模型标签与统计行，
     统计行是**本轮到此为止**的累计（耗时留空，整轮走完才确定）；
...
| 耗时口径 | **谁的时间就记在谁身上**，且一律由服务端测量：每段思维链记这一段思考的耗时（`thinking.duration_ms`，首帧思考增量到该段结束）；每次工具调用记执行耗时（回执里的 `duration_ms`）；每次模型请求的墙钟也照测并随消息下发（`usage.duration_ms`，发起到流读完，供其他客户端/后续下钻使用，当前界面不显示）。界面底部那一行的 `耗时` 是**整轮墙钟**（提问到收尾），由消息时间戳推算——它和「某个折叠头花了几秒」不是一回事，各自标在自己的位置上，不再互相顶替。 |
| 助手侧按轮分组 | 一轮（一条用户消息到下一轮开始）在界面上渲染成**一组**：顶部一个模型标签、中间按到达顺序排开的全部思考/工具/正文、底部一行整轮合计。理由：一次工具循环会落十几到几十条助手消息，逐条渲染会把同一轮的模型名与统计行重复十几遍（实测一屏全是重复的 `DeepSeek V4.1 Flash` + 统计行），既占高度又让人以为「每一步都花了这么多」。分组只在渲染层做，存储仍是一条请求一条消息（用量、模型、耗时都按请求记，才能算得出整轮合计）。 |
