...
>   第一个工具调用出现）时立刻广播该段墙钟，界面不必等整轮回读就能在思考折叠头显示耗时；
>   纯思考收尾（流里始终没有正文/工具调用）在流结束时补发一次
> - **`ActiveTurnCatchUp` 增加 `thinking_duration_ms`**：中途接入的客户端靠它回放最近一段
>   已结束思考的耗时，不必等下一条事件
> - **耗时文案最小单位到毫秒**：不足 1 秒显示为 `318ms`（整轮统计行为 `318 ms`），
>   不再四舍五入成 `0.3s` / `0.0 s`
...
pub enum AgentEvent {
    ...
    // 一段思维链结束：该段墙钟（毫秒），由运行时测量。段一结束就广播
    // （第一段正文 / 第一个工具调用出现，或纯思考时流结束），
    // 界面折叠头在流式期间即可显示耗时，不必等整轮回读
    ThinkingFinished {
        duration_ms: u64,
    },
    TextDelta { 
...
- 具备功能：
  1. 会话列表管理与工作区选择；
  2. 树状对话流展示、Markdown 渲染、Thinking 思维链折叠。**一个完整的回复（一轮）
     只显示一次模型名与一次统计行**：开头是模型标签，中间是按到达顺序排开的
     思考/工具/正文（步骤自身不再重复模型名与统计行），末尾一行
     `耗时 <整轮墙钟> · 该轮各次请求的用量之和`。**每个思考段与每次工具调用
     各自在折叠按钮左侧显示自己的耗时**（`thinking.duration_ms` / 回执里的
     `duration_ms`，都由服务端测量）：思考段一结束就随 `ThinkingFinished` 广播，
     流式期间折叠头即可显示，不必等整轮回读。**耗时最小单位是毫秒**，不足 1 秒
     给 `318ms`（整轮统计行为 `318 ms`），不四舍五入成 `0.3s` / `0.0 s`；
     流式中的那一轮同布局，统计行先给 **本轮到此为止**的累计、耗时留空，整轮走完才落定；
...
| 耗时口径 | 只保留两处，且都由服务端测量或按服务端时间戳推算：**一轮一次**的整轮墙钟（`耗时`，提问到收尾，按消息时间戳推算，显示在该轮末尾的统计行上）；**每个折叠头自己**的耗时（思考段 `thinking.duration_ms` 首帧到该段结束、工具调用回执里的 `duration_ms`）。两处最小单位都是毫秒：不足 1 秒给 `318ms` / `318 ms`，不四舍五入成 `0.3s` / `0.0 s`。思考段耗时在**段结束时**即随 `ThinkingFinished` 广播（纯思考收尾在流结束时补发），流式期间折叠头就能显示，不依赖整轮回读；中途接入的客户端由 `ActiveTurnCatchUp.thinking_duration_ms` 回放。曾经为「每条消息那次请求」也测过一份墙钟（`usage.duration_ms`），但界面既然只在一轮末尾显示一次耗时，那份数据没有去处，已连同测量代码删除。 |
| 助手侧按轮分组 | 一个完整回复 = 一轮（一条用户消息到下一轮开始），在界面上渲染成**一组**，**模型名与统计行各只出现一次**：顶部一个模型标签、中间按到达顺序排开的全部思考/工具/正文、底部一行整轮合计。理由：一次工具循环会落十几到几十条助手消息，逐条渲染会把同一轮的模型名与统计行重复十几遍（实测一屏全是重复的 `DeepSeek V4.1 Flash` + 统计行），既占高度又让人以为「每一步都花了这么多」。分组只在渲染层做，存储仍是一条请求一条消息（模型与用量按请求记，才能算得出整轮合计）。 |
