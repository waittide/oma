...
- 具备功能：
  ...
  2. 树状对话流展示、Markdown 渲染、Thinking 思维链折叠。**一个完整的回复（一轮）
     ...
     `duration_ms`，都由服务端测量）。流式中的那一轮同布局，统计行先给
     **本轮到此为止**的累计、耗时留空，整轮走完才落定；
...
| 耗时口径 | 只保留两处，且都由服务端测量或按服务端时间戳推算：**一轮一次**的整轮墙钟（`耗时`，提问到收尾，按消息时间戳推算，显示在该轮末尾的统计行上）；**每个折叠头自己**的耗时（思考段 `thinking.duration_ms` 首帧到该段结束、工具调用回执里的 `duration_ms`）。曾经为「每条消息那次请求」也测过一份墙钟（`usage.duration_ms`），但界面既然只在一轮末尾显示一次耗时，那份数据没有去处，已连同测量代码删除。 |
