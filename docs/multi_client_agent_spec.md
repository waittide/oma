...
- 具备功能：
  1. 会话列表管理与工作区选择；
  2. 树状对话流展示、Markdown 渲染、Thinking 思维链折叠。**助手侧一个模型请求一块**：
     各自的模型标签 + 各自的思考/工具/正文 + 各自的统计行（`耗时 <该次请求> · 该次请求
     的用量`）；同一轮内的后续块收窄上边距，读成一条连续的过程而不是一叠独立卡片。
     **一轮的最后一块换成整轮口径**（`耗时 <整轮墙钟> · 该轮各次请求用量之和`）：
     中间步骤的量本身很小，整轮的量只在该轮收尾处给一次。**每个思考段与每次工具调用
     各自在折叠按钮左侧显示自己的耗时**（`thinking.duration_ms` / 回执里的
     `duration_ms`，都由服务端测量）。流式中的那块与中间步骤同口径：显示最近一次
     完成的请求的耗时与用量（`usage_updated` / 追赶快照），与回读后落在该消息上的
     数字一致；
...
| 耗时口径 | **谁的时间就记在谁身上**，且一律由服务端测量：每条助手消息记该次模型请求的墙钟（`usage.duration_ms`，发起到流读完）；每段思维链记这一段思考的耗时（`thinking.duration_ms`，首帧思考增量到该段结束）；每次工具调用记执行耗时（回执里的 `duration_ms`）。中间步骤各自显示自己的那一次请求；**整轮墙钟**（提问到收尾，按消息时间戳推算）只出现在该轮最后一块的统计行上，与「这一轮总共花了多久」对应，不再贴到中间步骤上让人误读成单步耗时。 |
| 助手侧排版 | 一个模型请求一块：各自的模型标签 + 各自的思考/工具/正文 + 各自的统计行；同一轮内的后续块收窄上边距（`.msg.assistant.follow`），读成一条连续的过程而不是一叠独立卡片。**一轮的最后一块换整轮口径**：`耗时` 为该轮墙钟、用量为该轮各次请求之和——中间步骤的量本身很小，整轮的量只在该轮收尾处给一次。曾经试过把整轮并成一块（一个标签 + 一行合计），但那样每个步骤的模型归属和单步开销就看不见了，退回按请求分块。 |
